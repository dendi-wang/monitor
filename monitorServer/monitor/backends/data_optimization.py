    ...
    def process_and_save(self):
        ...
        if self.data['status'] == 0:  # service data is valid
            for key, data_series_val in settings.STATUS_DATA_OPTIMIZATION.items():
                ...
                if data_series_val[
                    0] == 0:  # this dataset is for unoptimized data, only the latest data no need optimiaztion
                    ...
                    last_point_data, last_point_save_time = json.loads(
                        self.redis_conn_obj.lrange(data_series_key_in_redis, -1, -1)[0])

                    if time.time() - last_point_save_time >= data_series_val[
                        0]:  # reached the data point update interval ,
