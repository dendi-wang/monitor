...
def test(request, arg):
    print arg
    # triggerobj = models.Trigger.objects.get(name='cpu50')
    # t = models.TriggerExpression.objects.filter(trigger=triggerobj)
    # for item in t:
    #     print item,item.logic_type
    return HttpResponse('ok')


def test(request):
    hostobj = models.Host.objects.get(ip_addr='192.168.112.130')
    print hostobj.host_groups.all(), type(hostobj.host_groups.all())
    print hostobj.templates.all(), type(hostobj.templates.all())
    # hostgrpobj = hostobj.host_groups.all()
    hosttmponj = hostobj.templates.all()
    print hostobj.templates.all(),
    for hostgrpobj in hostobj.host_groups.all():
        print hostgrpobj.templates.all()
    return HttpResponse('ok')
